...
public:
	...
	ServiceIndex_t &GetServiceCount(int dayIndex, int slotIndex);
	string &GetServiceName(int dayIndex, int slotIndex);

	void Load(const WeekSchedule &schedule);
	void Save(AppData &appData, WeekSchedule &schedule);
